...
![스크린샷 2024-07-15 오후 1 35 51](https://gist.github.com/user-attachments/assets/78f9ddf7-4aec-4a88-a3e3-d3793453e5ae)

- AJAX 요청을 만들면 네트워크 엔진이 AJAX 요청을 처리하기 위해 요청 큐에 넣는다.
...
=> 클릭 이벤트는 항상 작업 큐에 콜백 함수와 함께 들어간다. 이벤트 루프는 작업 큐를 확인하고 콜백 함수를 실행하는데, 이때 콜백 함수가 AJAX 요청을 처리하는 함수라면 요청 큐에 AJAX 요청을 추가한다. 네트워크 엔진이 요청 큐에 있는 요청을 꺼내서 처리한다.
...
비동기 호출에서는 결과값을 리턴값으로 줄 수 없다. 동기화된 함수와 같이 일반적인 방법으로 결과값을 리턴하면 안된다. 대신 콜백을 사용해 결과값을 전달할 수 있다.

결과가 준비되었을 때 결과를 인자에 넣어 콜백을 호출한다. 일반적인 자바스크립트에서 사용하는 비동기 프로그래밍 방법이다.
